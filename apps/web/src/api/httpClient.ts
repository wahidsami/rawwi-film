  ...
  if (path.startsWith('/tasks')) {
    ...
    if (method === 'PATCH') {
      ...
      return {
        ...
        currentStage: body?.currentStage ?? 'Initialization',
        reasoningTraceStage: body?.reasoningTraceStage ?? null,
        gcamMappingStatus: body?.gcamMappingStatus ?? 'Pending',
        knowledgeUsage: body?.knowledgeUsage ?? null,
        progressTotal: 10,
        progressDone: 4,
        progressPercent: 40,
        createdAt: new Date().toISOString(),
