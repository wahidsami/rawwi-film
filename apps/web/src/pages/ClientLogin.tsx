          ...
          <div className="flex-1 p-8 sm:p-10 relative">
            ...
            <div className="max-w-sm mx-auto mt-6 space-y-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <Input label={t('email')} type="email" value={email} onChange={(e) => setEmail(e.target.value)} required dir="ltr" autoComplete="email" />
                <Input label={t('password')} type="password" value={password} onChange={(e) => setPassword(e.target.value)} required dir="ltr" autoComplete="current-password" />
                {error && <div className="p-2.5 text-xs text-error bg-error/10 border border-error/20 rounded-md">{error}</div>}

                <div className="flex items-center justify-between text-xs">
                  <Link to="/client/forgot-password" className="text-primary hover:text-primary-hover font-medium">{t('forgotPassword')}</Link>
                  <Link to="/portal/register" className="text-primary hover:text-primary-hover font-medium">
                    {lang === 'ar' ? 'ليس لديك حساب؟ سجّل' : "Don't have an account? Register"}
                  </Link>
                </div>
