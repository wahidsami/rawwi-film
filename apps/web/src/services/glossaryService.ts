...
export async function exportGlossaryPdf(params: GlossaryExportPdfParams = {}): Promise<Blob> {
  ...
  if (!res.ok) {
    ...
    const msg = res.status === 410
      ? (params.lang === 'ar' ? 'تصدير PDF للمعجم متوفر من داخل التطبيق فقط.' : 'PDF export for glossary is available from the in-app view only.')
      : (err?.message || err?.error || 'Glossary PDF export failed');
    throw new Error(msg);
  }
  return res.blob();
}
