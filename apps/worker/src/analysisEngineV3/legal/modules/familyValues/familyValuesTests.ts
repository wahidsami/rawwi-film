import { strict as assert } from "node:assert";
import { buildIntelligenceContext } from "../../../intelligence/intelligenceBuilder.js";
import type { IntelligenceBuilderInput } from "../../../intelligence/intelligenceContext.js";
import { createLegalContextResult } from "../../legalContext.js";
import { createLegalEvidenceResult } from "../../legalEvidence.js";
import { createLegalEngine } from "../../legalEngine.js";
import { createLegalModuleLoader } from "../../legalModuleLoader.js";
import { LegalModuleRegistry } from "../../legalModuleRegistry.js";
import type { LegalModuleEvaluationInput } from "../../legalModule.js";
import { FAMILY_VALUES_EXAMPLES } from "./familyValuesExamples.js";
import { FAMILY_VALUES_MODULE, isFamilyValuesEvidenceText } from "./familyValuesModule.js";

function makeBaseInput(text: string): IntelligenceBuilderInput {
  return {
    moduleId: FAMILY_VALUES_MODULE.id,
    storyMemory: "السياق حاضر.",
    narrative: {
      speaker: "A",
      listener: "B",
      target: "B",
      narrativeVoice: "dialogue",
      sceneType: "family scene",
      narrativeIntent: "family_values",
      storyPosition: "conflict",
      relationship: "family",
      emotionalTone: "hostile",
      condemnation: false,
      approval: false,
      neutrality: false,
      historicalContext: false,
      dream: false,
      flashback: false,
      comedy: false,
      satire: false,
      threat: false,
      instruction: false,
      news: false,
      documentary: false,
      dialogue: true,
      narration: false,
      sceneDescription: false,
      confidence: 0.96,
    },
    evidence: createLegalEvidenceResult({
      candidates: [
        {
          text,
          startOffset: 0,
          endOffset: text.length,
          confidence: 0.99,
          source: "chunk",
        },
      ],
      primaryCandidateIndex: 0,
      admissible: true,
      confidence: 0.99,
    }),
    semantic: {
      semanticMeaning: text,
      narrativeIntent: "family_values",
      conversationRole: "speaker",
      sceneRole: "dialogue",
      speaker: "A",
      listener: "B",
      target: "B",
      victim: "B",
      emotion: "hostile",
      riskContext: "high",
      confidence: 0.97,
    },
    context: createLegalContextResult({
      storyMemory: "السياق حاضر.",
      sceneMemory: "مشهد عائلي",
      localContext: text,
      chunkContext: "Chunk 1",
      neighboringSentences: ["قبلها كان هناك توتر.", "بعدها ساد الصمت."],
      narrativeContext: "Family context.",
      confidence: 0.95,
    }),
    glossary: {
      title: "Test glossary",
      entries: [],
      notes: [],
    },
  };
}

function makeInput(text: string, overrides?: Partial<IntelligenceBuilderInput>): LegalModuleEvaluationInput {
  const base = makeBaseInput(text);
  return {
    moduleId: FAMILY_VALUES_MODULE.id,
    intelligence: buildIntelligenceContext({
      ...base,
      ...overrides,
      evidence: overrides?.evidence ?? base.evidence,
      narrative: overrides?.narrative ?? base.narrative,
      semantic: overrides?.semantic ?? base.semantic,
      context: overrides?.context ?? base.context,
      glossary: overrides?.glossary ?? base.glossary,
      storyMemory: overrides?.storyMemory ?? base.storyMemory,
    }),
  };
}

function evaluate(input: LegalModuleEvaluationInput) {
  const registry = new LegalModuleRegistry().register(FAMILY_VALUES_MODULE);
  const engine = createLegalEngine(createLegalModuleLoader(registry));
  return engine.evaluate(input);
}

function testRegistryAndHelpers(): void {
  const registry = new LegalModuleRegistry().register(FAMILY_VALUES_MODULE);
  assert.equal(registry.load(FAMILY_VALUES_MODULE.id)?.id, FAMILY_VALUES_MODULE.id);
  assert.equal(isFamilyValuesEvidenceText("هذه إساءة للأب والبيت"), true);
  assert.equal(isFamilyValuesEvidenceText("جلسوا يتحدثون"), false);
  assert.ok(FAMILY_VALUES_EXAMPLES.positive.length > 0);
  assert.ok(FAMILY_VALUES_EXAMPLES.negative.length > 0);
  console.log("✓ family values module registry and helpers");
}

function testFamilyDestruction(): void {
  const decision = evaluate(makeInput("هذا يدمّر العائلة ويهدم البيت"));
  assert.equal(decision.status, "accept");
  assert.equal(decision.finding !== null, true);
  console.log("✓ family destruction");
}

function testFamilyAbuse(): void {
  const decision = evaluate(makeInput("يمارس إساءة أسرية على الطفل"));
  assert.equal(decision.status, "accept");
  assert.equal(decision.finding !== null, true);
  console.log("✓ family abuse");
}

function testFamilyNeglect(): void {
  const decision = evaluate(makeInput("هذا إهمال أبوي واضح للطفل"));
  assert.equal(decision.status, "accept");
  assert.equal(decision.finding !== null, true);
  console.log("✓ parental neglect");
}

function testEducationalContext(): void {
  const decision = evaluate(
    makeInput("المعلم يشرح لماذا الإهمال الأبوي خطأ", {
      narrative: {
        speaker: "Teacher",
        listener: "Students",
        target: "Students",
        narrativeVoice: "narration",
        sceneType: "classroom",
        narrativeIntent: "education",
        storyPosition: "setup",
        relationship: null,
        emotionalTone: "neutral",
        condemnation: false,
        approval: false,
        neutrality: true,
        historicalContext: false,
        dream: false,
        flashback: false,
        comedy: false,
        satire: false,
        threat: false,
        instruction: true,
        news: false,
        documentary: false,
        dialogue: false,
        narration: true,
        sceneDescription: true,
        confidence: 0.95,
      },
      semantic: {
        ...makeBaseInput("المعلم يشرح لماذا الإهمال الأبوي خطأ").semantic,
        semanticMeaning: "Educational explanation of parental neglect.",
        narrativeIntent: "education",
        sceneRole: "educational",
        confidence: 0.95,
      },
      context: createLegalContextResult({
        storyMemory: "Educational scene.",
        sceneMemory: "Classroom.",
        localContext: "المعلم يشرح لماذا الإهمال الأبوي خطأ",
        chunkContext: "Chunk 1",
        neighboringSentences: ["هذا درس توعوي."],
        narrativeContext: "Educational context.",
        confidence: 0.94,
      }),
    }),
  );
  assert.equal(decision.status, "reject");
  assert.equal(decision.finding, null);
  console.log("✓ educational context");
}

function testNoSignal(): void {
  const decision = evaluate(
    makeInput("جلسوا يتحدثون عن العمل والسفر.", {
      narrative: {
        speaker: "A",
        listener: "B",
        target: null,
        narrativeVoice: "dialogue",
        sceneType: "conversation",
        narrativeIntent: "neutral",
        storyPosition: "setup",
        relationship: "friend",
        emotionalTone: "neutral",
        condemnation: false,
        approval: false,
        neutrality: true,
        historicalContext: false,
        dream: false,
        flashback: false,
        comedy: false,
        satire: false,
        threat: false,
        instruction: false,
        news: false,
        documentary: false,
        dialogue: true,
        narration: false,
        sceneDescription: false,
        confidence: 0.9,
      },
      semantic: {
        ...makeBaseInput("جلسوا يتحدثون عن العمل والسفر.").semantic,
        semanticMeaning: "Neutral conversation about work and travel.",
        narrativeIntent: "neutral",
        sceneRole: "dialogue",
        emotion: "neutral",
        confidence: 0.9,
      },
      context: createLegalContextResult({
        storyMemory: "Neutral discussion.",
        sceneMemory: "Office talk.",
        localContext: "جلسوا يتحدثون عن العمل والسفر.",
        chunkContext: "Chunk 1",
        neighboringSentences: ["لا يوجد شيء مهم هنا."],
        narrativeContext: "Neutral conversation.",
        confidence: 0.9,
      }),
    }),
  );
  assert.equal(decision.status, "reject");
  assert.equal(decision.finding, null);
  console.log("✓ no family signal");
}

function testDeterministicOutput(): void {
  const first = evaluate(makeInput("هذا يدمّر العائلة ويهدم البيت"));
  const second = evaluate(makeInput("هذا يدمّر العائلة ويهدم البيت"));
  assert.equal(JSON.stringify(first), JSON.stringify(second));
  console.log("✓ deterministic output");
}

async function main(): Promise<void> {
  testRegistryAndHelpers();
  testFamilyDestruction();
  testFamilyAbuse();
  testFamilyNeglect();
  testEducationalContext();
  testNoSignal();
  testDeterministicOutput();
  console.log("\nAll family values module tests passed.");
}

main().catch((error) => {
  console.error(error);
  process.exit(1);
});
