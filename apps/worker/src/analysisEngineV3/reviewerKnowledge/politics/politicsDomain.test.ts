/**
 * Politics domain validation.
 * Run: node --import tsx apps/worker/src/analysisEngineV3/reviewerKnowledge/politics/politicsDomain.test.ts
 */
...
const BLUEPRINT_ROOT = join(ROOT, "blueprints", "politics");
const ACADEMY_PACK = join(ROOT, "academy", "politics", "pack.v1.json");
const LESSON_ROOT = join(ROOT, "lessons", "universal");
const PATTERN_ROOT = join(ROOT, "patternLibraries", "politics");
const DECISION_ROOT = join(ROOT, "decisionRecords", "examples", "politics");
const DECISION_VALIDATION_ROOT = join(ROOT, "decisionRecords", "examples");
const BENCHMARK_FILE = join(ROOT, "benchmarks", "politics", "politics_benchmark_cases.v1.json");
const COVERAGE_FILE = join(ROOT, "blueprints", "politics", "politics_coverage_report.json");
...
function testDecisionRecords() {
  ...
  assert.equal(validation.valid, true, validation.issues.map((issue) => issue.message).join("; "));
  assert.equal(searchDecisionRecords(records, { concept: "state_reference" }).length >= 1, true);
}
...
function testCoverage() {
  ...
  const report = analyzer.analyze("politics");
  mkdirSync(join(ROOT, "blueprints", "politics"), { recursive: true });
  ...
  assert.equal(discoverDomainCoverageDomains().includes("politics"), true);
}
...
function main() {
  ...
  console.log("All politics domain tests passed.");
}
