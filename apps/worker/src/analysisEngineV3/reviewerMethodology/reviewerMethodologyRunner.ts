...
function inferNarrativeIntent(input: ReviewerMethodologyRunnerInput): string {
  const text = normalizeText(input.promptInput.chunkContext.localChunk);
...
export function runReviewerMethodology(input: ReviewerMethodologyRunnerInput): ReviewerAssessment {
  ...
  const stageResults = Object.freeze([
    buildStageResult(
      "evidence_extraction",
      "Evidence Extraction",
      "Locate and freeze the smallest grounded evidence span.",
      "complete",
      chunkText,
      0.9,
      ["chunk"],
    ...
    buildStageResult(
      "evidence_judge",
      "Evidence Judge",
      "Record the literal facts visible in the grounded evidence.",
      "complete",
      reasoningTrace.join(" | "),
      evidenceStrength,
      ["grounded_evidence"],
    ...
    buildStageResult(
      ...
      "Extract legal concepts from grounded evidence without naming the final GCAM article.",
      input.conceptContext.conceptCount > 0 ? "complete" : "uncertain",
      input.conceptContext.conceptIds.length > 0 ? input.conceptContext.conceptIds.join(", ") : "No canonical concepts detected.",
      input.conceptContext.conceptCount > 0 ? 0.93 : 0.38,
      ["grounded_evidence", "observed_facts"],
      ["concepts", "knowledge_domains"],
    ),
    buildStageResult(
      "legal_classification",
      "Legal Classification",
      "Rank the legal consequence of the grounded concepts using Academy knowledge.",
      input.conceptContext.conceptCount > 0 ? "complete" : "partial",
      input.conceptContext.conceptIds.length > 0 ? input.conceptContext.conceptIds.join(", ") : "No candidate articles selected yet.",
      input.conceptContext.conceptCount > 0 ? 0.91 : 0.42,
      ["grounded_evidence", "observed_facts", "concepts", "knowledge_domains"],
      ["primary_article", "secondary_articles", "applicable_atoms"],
    ),
    buildStageResult(
      "explanation",
      "Explanation",
      "Generate an explanation that only references grounded evidence and the selected article.",
      "complete",
      reasoningTrace.join(" | "),
      average([evidenceStrength, conceptConfidence, exceptionSignals.length > 0 ? 0.7 : 0.82]),
      ["grounded_evidence", "concepts", "primary_article", "secondary_articles"],
      ["explanation"],
    ),
    buildStageResult(
      ...
      reasoningTrace.join(" | "),
      average([evidenceStrength, conceptConfidence, exceptionSignals.length > 0 ? 0.7 : 0.82]),
      ["grounded_evidence", "concepts", "primary_article", "explanation"],
      ["validated_finding"],
    ),
  ]);
