...
export function buildRankingSignalTerms(input: ReviewerRankingBaseInput, scopeCategories: readonly string[]): readonly string[] {
  return uniqueNonEmpty([
...
export function buildReviewerRankingDiagnostics(input: Readonly<{
  enabled: boolean;
  ...
  return Object.freeze({
    ...
    selectedReviewerCount: input.selectedReviewerFolders.length,
    selectedArticleCount: input.articleRanking.selectedArticleCount,
    rejectedArticleCount: input.articleRanking.rejectedArticleCount,
    selectedAtomCount: input.atomRanking.selectedAtomCount,
    rejectedAtomCount: input.atomRanking.rejectedAtomCount,
    articleReductionPercent: input.articleRanking.articleReductionPercent,
    atomReductionPercent: input.atomRanking.atomReductionPercent,
    selectedArticleIdsByReviewer: input.articleRanking.selectedArticleIdsByReviewer,
    selectedAtomIdsByArticle: input.atomRanking.selectedAtomIdsByArticle,
    topArticleScores: Object.freeze(input.articleRanking.articleScores.filter((item) => item.selected).slice(0, 10)),
    topAtomScores: Object.freeze(input.atomRanking.atomScores.filter((item) => item.selected).slice(0, 10)),
  });
}
