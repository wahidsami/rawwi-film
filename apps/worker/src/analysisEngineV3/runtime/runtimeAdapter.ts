...
function defaultOutputSchema(): V3PromptOutputSchema {
  return {
    ...
    fields: [
      ...
      { name: "reasoned_decision", description: "GPT Reviewer Assistant reasoning package with reasoning, article evaluations, supporting and contradicting evidence, applicable and rejected articles, risk analysis, narrative analysis, human-like explanation, and confidence.", required: true },
    ],
    notes: [
      ...
      "The reasoned decision must answer reasoning, articleEvaluations, supportingEvidence, contradictingEvidence, applicableArticles, rejectedArticles, riskAnalysis, narrativeAnalysis, humanLikeExplanation, recommendation, and confidence.",
      "Prefer explicit articleEvaluations for every independent violation evidence unit. Article ids may repeat across evaluations when different grounded violations map to the same GCAM article. applicableArticles is only the unique PASS summary of those evaluations.",
      ...
      "Evaluate every GCAM article independently and return PASS or FAIL for each one. Preserve repeated article evaluations when separate evidence units support the same article.",
