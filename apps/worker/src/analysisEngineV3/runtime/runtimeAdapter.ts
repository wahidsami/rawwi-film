...
function defaultOutputSchema(): V3PromptOutputSchema {
  return {
    ...
    fields: [
      ...
      { name: "reasoned_decision", description: "GPT Reviewer Assistant reasoning package with legal concepts, knowledge domains, candidate GCAM articles, primary and secondary article ranking, reasoning, article evaluations, supporting and contradicting evidence, applicable and rejected articles, risk analysis, narrative analysis, human-like explanation, and confidence.", required: true },
    ],
    notes: [
      "The evidence object must include candidates and primaryCandidateIndex.",
      "Evidence candidates are sentence-level and must be preserved independently when multiple suspicious statements appear in the chunk.",
      ...
      "The reasoned decision must be evidence-first, quote-grounded, concept-first, knowledge-domain aware, candidate-article aware, and non-hallucinatory.",
      ...
      "The reasoned decision must answer legalConcepts, knowledgeDomains, candidateArticles, primaryArticle, secondaryArticles, reasoning, articleEvaluations, supportingEvidence, contradictingEvidence, applicableArticles, rejectedArticles, riskAnalysis, narrativeAnalysis, humanLikeExplanation, recommendation, and confidence.",
      "Prefer explicit articleEvaluations for every independent violation evidence unit. Article ids may repeat across evaluations when different grounded violations map to the same GCAM article. applicableArticles is only the unique PASS summary of the ranked result.",
      "Do not invent facts, actors, objects, injuries, or events that are not present in the quoted evidence or current scene.",
      "Exception handling belongs to the deterministic post-processing policy engine, not to the reviewer output.",
      "Evaluate every GCAM article independently after concept analysis and candidate ranking. Preserve repeated article evaluations when separate evidence units support the same article.",
        ...
        articleEvaluations: [
          {
            ...
            reason: "The quote directly supports the semantic conclusion.",
            confidence: 0.95,
          },
          {
            articleId: 4,
  ...
  if (config.V3_INSPECTION_MODE) {
    try {
      ...
      const knowledgeRecord = buildV3KnowledgeMatchingInspectionRecord({
        ...
        detectedConceptIds: [...(routing.detectedConceptIds ?? [])],
        detectedConceptLabels: [...(routing.detectedConceptLabels ?? [])],
        knowledgeDomains: [...(routing.knowledgeDomains ?? [])],
        evidenceType: routing.evidenceType ?? null,
        sceneDescriptionType: routing.sceneDescriptionType ?? null,
        storyContextType: routing.storyContextType ?? null,
