...
function detectSurfaceConceptIds(corpusText: string): readonly string[] {
  ...
  const patterns: readonly (readonly [string, RegExp])[] = [
    ["profanity", /(?:يا[.…\.]{2,}|يا\s+(?:كلب|حمار|خرا|خنزير|غبي|حقير|قذر|سافل|وسخ|وسخة|لعين|نصاب|حرامي)|شتيمة|سباب|قذف)/u],
    ["insult", /(?:يا[.…\.]{2,}|يا\s+(?:كلب|حمار|خرا|خنزير|غبي|حقير|قذر|سافل|وسخ|وسخة|لعين|نصاب|حرامي)|أكرهك|أكرهكم|تافه|ساقط|حقير)/u],
    ["hostility", /(?:موتوا|موتي|اخرجوا|اخرجي|انقلع|انقلعي|أكرهك|أكرهكم|خلصوني منكم|سحقا|اذهبوا|اذهبي)/u],
...
function inferKnowledgeDomains(conceptIds: readonly string[], corpusText: string): readonly string[] {
  ...
  if (domains.size === 0 && (/(?:يا[.…\.]{2,}|يا\s+(?:كلب|حمار|خرا|خنزير|غبي|حقير|قذر|سافل|وسخ|وسخة|لعين))/u.test(text) || /(?:أكرهك|أكرهكم|خلصوني منكم|موتوا)/u.test(text))) {
    add("profanity");
  }
