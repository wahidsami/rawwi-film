...
function applyViolationSystemOverlay(prompt: string, passName: string): string {
  ...
  if (passName.startsWith("v3_")) return prompt;
...
const V3_SUBJECT_PASSES: PassDefinition[] = [
  {
    name: "glossary",
    articleIds: [],
    buildPrompt: buildGlossaryPrompt,
  ...
  ...V3_SUBJECT_DEFINITIONS.map((subject): PassDefinition => ({
    name: subject.name,
    articleIds: subject.articleIds,
    buildPrompt: (articles: GCAMArticle[]) => buildV3SubjectPrompt(subject, articles),
    model: subject.model,
  })),
];

export const DETECTION_PASSES: PassDefinition[] =
  config.VIOLATION_SYSTEM_VERSION === "v3" ? V3_SUBJECT_PASSES : LEGACY_DETECTION_PASSES;
