...
**On-demand highlight (Findings tab):** After you pick a report, findings **do not** auto-highlight in the script. Each card has **«Find in script»** (`بحث في النص وتمييز`): it searches the **full workspace text** (all pages, exact evidence first then fuzzy), jumps to the matching page, and highlights **only that** finding. **Clear highlight** / changing report resets it. Same **Script workspace** (`/workspace/:id`) is used for **Quick Analysis** and **client-assigned** scripts.
